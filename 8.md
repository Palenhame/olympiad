...
**URL**: `ws://127.0.0.1:8000/ws/pvp/matchmaking?subject=math`
...
**Поля**:
- `type` (string (enum), обязательно):  "is_search"
...
```json
{
  ...
  "task_order": 0,
  "me_is_correct": true,
  "enemy_is_correct": true,
  "next_task_order": 0
}
```

**Текущая статистика по PVP**
```json
{
	"type": "stats",
	"completion_percentage": 67,
	"correct_percentage": 52,
	"enemy_correct_percentage": 42,
	"current_task": 5
}
```
...
```json
{
  ...
  "my_new_rating": 1032,
  "enemy_delta": -32,
  "enemy_old_rating": 994,
  "enemy_new_rating": 1035
}
```
...
**Поля**:
- `type` (string): Тип сообщения ("result", "enemy_confirm", "confirm", "stats", "finish_round", "error")
- `task_index` (integer): Индекс задачи
- `*_is_correct` (boolean): Правильный ли ответ
- `errors` (string): Сообщение об ошибке (только для типа error)
-  `next_task_id` (long): ID следующего задания
- `completion_percentage` (integer): Процент выполнения всего pvp
- `*_correct_percentage` (integer): Процент правильности выполнения заданий (пользователем или соперником)
